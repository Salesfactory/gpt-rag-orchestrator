...
def crawl_website(
    url: str, limit: int = 30, max_depth: int = 4, max_breadth: int = 15
) -> Dict[str, Any]:
    ...
    try:
        ...
        _logger.info(
            f"Starting crawl for {url} with limit={limit}, depth={max_depth}, breadth={max_breadth}"
        )
        ...
        _logger.info(
            f"Successfully crawled {url}, found {len(response.get('results', []))} results"
        )
        return response
        ...
        return {"base_url": url, "results": [], "response_time": 0.0, "error": str(e)}


def crawl_website_and_save(
    url: str,
    output_file: str = "response.jsonl",
    limit: int = 30,
    max_depth: int = 4,
    max_breadth: int = 15,
) -> bool:
    ...
    try:
        ...
        if response.get("results"):
            try:
                ...
                _logger.info(
                    f"Successfully saved {len(response['results'])} results to {output_file}"
                )
                return True
            except IOError as e:
