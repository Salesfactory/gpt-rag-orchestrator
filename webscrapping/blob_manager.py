...
class WebCrawlerManager:
    ...
    def store_multipage_results_in_blob(
        self, formatted_pages: List[Dict[str, Any]], content_type: str = "text/plain"
    ):
        """
        ...
        for page_data in formatted_pages:
            try:
                ...
                blob_path = self._get_blob_path_for_multipage(
                    page_url, page_metadata, content_type
                )
                ...
                content_already_exists = self._content_exists_by_checksum(
                    checksum, page_url
                )
                ...
                if not content_already_exists:
                    ...
                    self._logger.debug(
                        "[BlobManager] Duplicate URL found for page: %s (URL already processed in this session)",
                        page_url,
                    )
                ...
                failed_uploads.append(
                    {"url": page_data.get("url", "unknown"), "error": str(e)}
                )
    ...
    def _get_blob_path_for_multipage(
        self, url: str, metadata: dict, content_type: str
    ) -> str:
        """
        ...
        blob_path = os.path.join(
            f"WebCrawler_Output/{base_blob_path}", f"{file_name}{file_extension}"
        )
