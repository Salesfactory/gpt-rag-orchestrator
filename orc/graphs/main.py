...
from orc.graphs.tools import (
    CustomRetriever,
    TavilySearch,
    retrieve_and_convert_to_document_format,
)
...
def _add_documents_to_dict(
    docs_dict: dict, documents: List[Document], source_type: str
) -> int:
    """
    ...
    for doc in documents:
        if hasattr(doc, "id") and doc.id:
            ...
            added_count += 1
            logger.debug(
                f"[Document Processing] 🔧 Used fallback key for {source_type} doc: {fallback_key[:50]}..."
            )
...
def _classify_document_source(doc: Document) -> str:
    ...
    if "http" in source and not any(
        domain in source for domain in ["blob.core.windows.net"]
    ):
        return "web"
    return "retrieval"
...
def clean_chat_history_for_agentic_search(chat_history: List[dict]) -> List[dict]:
    ...
    logger.info("[Chat History Cleaning] Formatted for agentic search mode")
...
class GraphBuilder:
    ...
    def _init_retriever(self) -> CustomRetriever:
        logger.info(
            "[GraphBuilder Retriever Init] Initializing Custom Agentic Retriever"
        )
        try:
            config = self.config
            ...
            if not index_name:
                logger.error(
                    "[GraphBuilder Retriever Init] AZURE_AI_SEARCH_INDEX_NAME is not set"
                )
                raise ValueError(
            ...
            logger.info(
                f"[GraphBuilder Retriever Init] Successfully initialized Custom Agentic Retriever with index: {index_name}, topK: {config.retriever_top_k}"
            )
            return retriever
        except Exception as e:
            logger.error(
                f"[GraphBuilder Retriever Init] Failed to initialize Azure AI Search Retriever: {str(e)}"
            )
            raise RuntimeError(
...
#! TBR (to be removed)
    def _run_agentic_retriever(self, conversation_history: List[dict]):
        ...
        logger.info(
            f"[GraphBuilder Agentic Retriever] Running agentic retriever with {len(conversation_history)} messages"
        )
        results = retrieve_and_convert_to_document_format(
            conversation_history, self.organization_id
        )
        # Safe handling of potentially None results
        results_count = len(results) if results else 0
        logger.info(
            f"[GraphBuilder Agentic Retriever] Retrieved {results_count} documents"
        )
        return results or []
...
#! TBR (to be removed)
    async def _execute_single_query_async(
        ...
        async with semaphore if semaphore else asyncio.Semaphore(4):
            try:
                logger.info(
                    f"[Async Custom Agentic Search] 🔍 Starting {query_type}: {query_text}"
                )
                ...
                retrieval_results = await loop.run_in_executor(
                    None, self.retriever.invoke, query_text
                )
                ...
                if needs_web_search:
                    logger.info(
                        f"[Async Custom Agentic Search] 🌐 {query_type} retrieved {len(retrieval_results)} documents - performing web search"
                    )
                    try:
                        # Perform web search for this specific query
                        web_results = await loop.run_in_executor(
                            None, self.web_search.search, query_text
                        )
                        logger.info(
                            f"[Async Custom Agentic Search] 🌐 {query_type} web search completed - {len(web_results)} documents"
                        )
                    except Exception as web_error:
                        logger.error(
                            f"[Async Custom Agentic Search] ❌ {query_type} web search failed: {str(web_error)}"
                        )
                        web_results = []
                else:
                    logger.info(
                        f"[Async Custom Agentic Search] ✅ {query_type} retrieved {len(retrieval_results)} documents - no web search needed"
                    )
                ...
                logger.info(
                    f"[Async Custom Agentic Search] ✅ {query_type} completed in {execution_time:.2f}s - {total_docs} total documents ({len(retrieval_results)} retrieval + {len(web_results)} web)"
                )
                ...
                execution_time = time.time() - start_time
                logger.error(
                    f"[Async Custom Agentic Search] ❌ {query_type} failed in {execution_time:.2f}s: {str(e)}"
                )
                ...
                if "capacity" in str(e).lower() or "overload" in str(e).lower():
                    ...
                    logger.warning(
                        f"[Async Custom Agentic Search] ⏳ Capacity issue detected, backing off for {backoff_delay}s"
                    )
...
#! TBR (to be removed)
    async def _execute_queries_async(self, sub_queries: List[str]) -> tuple:
        ...
        for i, query in enumerate(sub_queries, 1):
            query_type = (
                "Rewritten Query" if i == len(sub_queries) else f"SUB-QUERY {i}"
            )
            query_tasks.append((i, query, query_type))

        logger.info(
            f"[Async Custom Agentic Search] 🚀 EXECUTING {len(query_tasks)} QUERIES ASYNCHRONOUSLY"
        )
        logger.info("\n" + "=" * 80)
        ...
        # Create async tasks
        tasks = [
            self._execute_single_query_async(task, semaphore) for task in query_tasks
        ]
        ...
        for result in results:
            if isinstance(result, Exception):
                logger.error(
                    f"[Async Custom Agentic Search] ❌ Task failed with exception: {result}"
                )
                continue
            ...
            logger.info(
                f"[Async Custom Agentic Search] ├─ Execution time: {execution_time:.2f}s"
            )
            logger.info(
                f"[Async Custom Agentic Search] ├─ Total documents: {results_count}"
            )
            logger.info(
                f"[Async Custom Agentic Search] ├─ Retrieval documents: {len(retriever_results)}"
            )
            logger.info(
                f"[Async Custom Agentic Search] ├─ Web search documents: {len(web_results)}"
            )

            # Log document sources if available
            if all_docs:
                logger.info("[Async Custom Agentic Search] ├─ 📄 All documents:")
                ...
                if retriever_results:
                    ...
                    for j, doc in enumerate(retriever_results, 1):
                        ...
                        logger.info(
                            f"[Async Custom Agentic Search] │  │  └─ {j}. Source: {source}"
                        )

                # Log web search documents
                if web_results:
                    logger.info(
                        "[Async Custom Agentic Search] │  └─ 🌐 From web search:"
                    )
                ...
                logger.info(
                    "[Async Custom Agentic Search] ├─ ❌ No documents found for this query"
                )

            logger.info(
                f"[Async Custom Agentic Search] └─ {query_type} PROCESSING COMPLETED"
            )
            logger.info("\n" + "=" * 80)

            # Add documents to deduplication dictionary
            logger.info(
                f"[Async Custom Agentic Search] ├─ 🔄 Adding {len(all_docs)} documents to deduplication dict"
            )

            retrieval_added = _add_documents_to_dict(
                docs_dict, retriever_results, "retrieval"
            )
            web_added = _add_documents_to_dict(docs_dict, web_results, "web")

            logger.info(
                f"[Async Custom Agentic Search] ├─ 📊 Added to dict: {retrieval_added} retrieval + {web_added} web = {retrieval_added + web_added} total"
            )
            logger.info(
                f"[Async Custom Agentic Search] ├─ 📋 Current dict size: {len(docs_dict)} unique documents"
            )
        ...
        logger.info(
            f"[Async Custom Agentic Search] 🔍 Web search usage detected: {'YES' if any_web_search_used else 'NO'}"
        )
...
#! TBR (to be removed)
    def _execute_custom_agentic_search(
        ...
        logger.info(
            f"[Custom Agentic Search Main] Generated {len(sub_queries)-1 if len(sub_queries) > 1 else len(sub_queries)} sub-queries for retrieval and added the rewritten query to the search list"
        )
        logger.info("\n" + "-" * 78)
        ...
        try:
            (
                ...
                parallel_total_time,
                any_web_search_used,
            ) = asyncio.run(self._execute_queries_async(sub_queries))

        except Exception as e:
            logger.error(f"[Custom Agentic Search Main] ❌ Async execution failed: {e}")
            logger.error(
                "[Custom Agentic Search Main] 💥 Async execution is required for custom agentic search"
            )
        ...
        logger.info("\n" + "=" * 78)
        logger.info("[Custom Agentic Search Main] 📊 EXECUTION SUMMARY:")
        logger.info(
            f"[Custom Agentic Search Main] ├─ Total queries executed: {len(sub_queries)}"
        )
        logger.info(
            f"[Custom Agentic Search Main] ├─ Execution time: {parallel_total_time:.2f}s"
        )
        logger.info(
            f"[Custom Agentic Search Main] ├─ Total documents retrieved: {total_retrieved}"
        )
        logger.info(
            f"[Custom Agentic Search Main] ├─ Unique documents after deduplication: {unique_docs}"
        )
        logger.info(
            f"[Custom Agentic Search Main] ├─ 📋 Final composition: {final_retrieval_count} retrieval + {final_web_count} web documents"
        )
        logger.info(
            f"[Custom Agentic Search Main] ├─ 🔍 Web search usage: {'Used by at least one query' if any_web_search_used else 'Not used by any query'}"
        )
        logger.info(
            "[Custom Agentic Search Main] ├─ 🎯 Per-query web search triggered for queries with <2 retrieval results"
        )
        logger.info("\n" + "=" * 78)
...
#! TBR (to be removed)
    def _init_web_search(self):
        ...
        try:
            ...
            logger.info(
                f"[GraphBuilder Web Search Init] Successfully initialized TavilySearch with max_results: {config.web_search_results}"
            )
            return web_search
        except Exception as e:
            logger.error(
                f"[GraphBuilder Web Search Init] Failed to initialize Tavily Web Search: {str(e)}"
            )
    ...
    def _return_state(self, state: ConversationState) -> dict:
        ...
        logger.info(
            f"[GraphBuilder Return State] Returning state with {messages_count} messages, {context_docs_count} context docs"
        )
        return {
    ...
    def _retrieve_context(self, state: ConversationState) -> dict:
        ...
        conversation_history = get_conversation_data(self.conversation_id).get(
            "history", []
        )
        # clean the conversation history
        conversation_history = clean_chat_history_for_agentic_search(
        ...
        logger.info("\n" + "=" * 78)
        logger.info("[Retrieve Context] 📋 POST-RETRIEVAL DECISION ANALYSIS:")
        ...
        logger.info("[Retrieve Context] └─ 🏁 RETRIEVAL PHASE COMPLETED")
        logger.info("\n" + "=" * 78)
