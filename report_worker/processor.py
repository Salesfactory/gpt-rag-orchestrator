...
async def process_report_job(
    ...
    if not job:
        error_msg = (
            f"Job {job_id} not found in database for organization {organization_id}"
        )
        logging.error(f"[ReportWorker] {error_msg}")
        raise ValueError(error_msg)

    logging.info(
        f"[ReportWorker] Retrieved job {job_id}: {job.get('report_key', 'unknown')}"
    )
    ...
    logging.info(
        f"[ReportWorker] Found generator for {report_key}: {generator.__class__.__name__}"
    )
    ...
    try:
        ...
        logging.info(
            f"[ReportWorker] Generated markdown content ({len(markdown_content)} chars) for job {job_id}"
        )
        ...
        logging.info(
            f"[ReportWorker] Generated PDF ({len(pdf_bytes)} bytes) for job {job_id}"
        )
        ...
        success = update_report_job_status(
            job_id, organization_id, "SUCCEEDED", result_metadata=result_metadata
        )

        if not success:
            logging.error(
                f"[ReportWorker] Failed to update job {job_id} status to SUCCEEDED"
            )
...
def generate_file_name(report_key: str, parameters: Dict[str, Any]) -> str:
    ...
    if report_key == "brand_analysis":
        ...
        category_name = "_".join(
            [
                x.get("category", "").replace(" ", "_")
                for x in parameters.get("categories", [])
            ]
        )
...
def extract_message_metadata(
    msg,
) -> Tuple[Optional[str], Optional[str], Optional[str], int, str]:
    """
    ...
    try:
        ...
        try:
            ...
            logging.error(
                f"[ReportWorker] Raw message: {repr(msg.get_body() if hasattr(msg, 'get_body') else msg)}"
            )
        ...
        if not all([job_id, organization_id]):
            logging.error(
                f"[ReportWorker] Missing required fields in payload: {payload}"
            )
            return None, None, None, dequeue_count, message_id
