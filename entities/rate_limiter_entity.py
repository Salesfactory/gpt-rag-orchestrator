...
def RateLimiter(ctx: df.DurableEntityContext):
    """
    ...
    if op == "acquire":
        ...
        if state["global_inflight"] < state["global_limit"] and tenant_in < tenant_limit:
            ...
            ctx.set_state(state)
            ctx.set_result({"granted": True})
        else:
            ctx.set_state(state)
            ctx.set_result({"granted": False, "wait_ms": 1500})
        ...
        ctx.set_result({"error": f"unknown op {op}"})
