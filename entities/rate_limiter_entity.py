...
def RateLimiter(context: df.DurableOrchestrationContext):
    ...
    if op == "acquire":
        ...
        tenant_limit = state["per_tenant_limit"].get(
            tenant, state["per_tenant_limit"]["default"]
        )
        tenant_in = state["inflight_by_tenant"].get(tenant, 0)

        if (
            state["global_inflight"] < state["global_limit"]
            and tenant_in < tenant_limit
        ):
        ...
        if state["global_inflight"] > 0:
            state["global_inflight"] -= 1
        if (
            tenant in state["inflight_by_tenant"]
            and state["inflight_by_tenant"][tenant] > 0
        ):
