...
# Page config
st.set_page_config(page_title="RAG Evaluation Tool", page_icon="RAG", layout="wide")
...
if uploaded_file:
    try:
        ...
        st.dataframe(
            df[["question", "fact", "source"]].head(10), use_container_width=True
        )
        ...
        with col2:
            st.info(
                "This will query the RAG endpoint for each question and evaluate responses."
            )
        ...
        if st.session_state.get("run_evaluation", False):
            ...
            try:
                ...
                for idx, row in df.iterrows():
                    status_text.text(
                        f"Processing question {idx + 1}/{total}: {row['question'][:50]}..."
                    )
                ...
                st.info(
                    "Make sure O1_KEY and O1_ENDPOINT environment variables are set."
                )
        ...
        if "results_df" in st.session_state:
            ...
            with col3:
                accurate_count = len(
                    results_df[results_df["factual_accuracy"] == "accurate"]
                )
            ...
            display_df = results_df[
                ["question", "fact", "ai_answer", "reasoning", "score"]
            ]
            st.dataframe(
                display_df,
                ...
                column_config={
                    ...
                    "score": st.column_config.NumberColumn(
                        "Score", format="%d", min_value=0, max_value=5
                    ),
                },
            )
            ...
            with col1:
                ...
                with pd.ExcelWriter(output, engine="openpyxl") as writer:
                    results_df.to_excel(
                        writer, index=False, sheet_name="Evaluation Results"
                    )
