...
class LLMConfig(BaseModel):
    ...
    def get_token(self):
        ...
        """
        logger.info(f"{LOGGER_NAME} Attempting to get bearer token for Azure OpenAI API")
        try:
            token = get_bearer_token_provider(
                DefaultAzureCredential(), "https://cognitiveservices.azure.com/.default")
            logger.info(f"{LOGGER_NAME} Successfully obtained bearer token")
            return token
        except Exception as e:
            logger.error(f"{LOGGER_NAME} Failed to get bearer token: {str(e)}", exc_info=True)
            raise
...
class LLMManager:
    def __init__(self, deployment_name: str):
        logger.info(f"{LOGGER_NAME} Initializing LLMManager with deployment: {deployment_name}")
        logger.info(f"{LOGGER_NAME} Setting up PromptTemplate and configuration")
        self.prompts = PromptTemplate()
        self._clients: Dict[str, Union[AzureOpenAI]] = {}
        self.config = LLMConfig(deployment_name=deployment_name)
        logger.info(f"{LOGGER_NAME} LLMManager initialization complete")
    
    def get_client(self, client_type: str, use_langchain: bool = False) -> Union[AzureOpenAI]:
        logger.info(f"{LOGGER_NAME} Getting client of type: {client_type} (use_langchain: {use_langchain})")
        ...
        """
        try:
            logger.info(f"{LOGGER_NAME} Creating standard Azure OpenAI client")
            client = AzureOpenAI(
                azure_ad_token_provider=self.config.get_token(), 
                api_version=self.config.api_version,
                azure_endpoint=self.config.api_base,
            )
            logger.info(f"{LOGGER_NAME} Successfully created client: {type(client).__name__}")
            self._clients[client_type] = client
            return client
        except Exception as e:
            logger.error(f"{LOGGER_NAME} Failed to create client: {str(e)}", exc_info=True)
            raise
    ...
    def get_prompt(self, prompt_type: str) -> str:
        ...
        Get a system prompt
        """
        logger.info(f"{LOGGER_NAME} Retrieving prompt of type: {prompt_type}")
        try:
            prompt = getattr(self.prompts, prompt_type)
            logger.info(f"{LOGGER_NAME} Successfully retrieved prompt: {prompt[:50]}...")
            return prompt
        except AttributeError as e:
            logger.error(f"{LOGGER_NAME} Unknown prompt type: {prompt_type}")
    ...
    @retry(
        ...
        before_sleep=lambda retry_state: logger.warning(f"{LOGGER_NAME} Retrying after error. Attempt {retry_state.attempt_number}/3")
    )
    def _make_chat_request(self, client, prompt, user_message):
        try:
            ...
            logger.error(f"{LOGGER_NAME} OpenAI API error: {str(e)}")
            raise
        except Exception as e:
            logger.error(f"{LOGGER_NAME} Unexpected error during chat completion: {str(e)}")
            raise
    ...
    def get_response(self, prompt_type: str = None, client_type: str = None, use_langchain: bool = False, custom_prompt: str = None, user_message: str = "Hello!") -> str:
        logger.info(f"{LOGGER_NAME} Getting response using prompt_type: {prompt_type}, client_type: {client_type}")
        try:
            ...
            logger.info(f"{LOGGER_NAME} Using standard Azure OpenAI client for request")
            return self._make_chat_request(client, prompt, user_message)
                
        except Exception as e:
            logger.error(f"{LOGGER_NAME} Error getting response: {str(e)}", exc_info=True)
            raise
