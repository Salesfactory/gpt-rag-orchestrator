...
def fetch_image_from_blob(image_path):
    ...
    try:
        ...
        logging.info(
            f"Successfully fetched image from container documents: {image_path}"
        )
        return image_data
    except Exception as e:
        logging.error(
            f"Error fetching image from blob storage ({image_path}): {str(e)}"
        )
        return None
...
def format_conversation_as_docx(conversation_data):
    ...
    try:
        assistant_style = doc.styles.add_style(
            "AssistantMessage", WD_STYLE_TYPE.PARAGRAPH
        )
