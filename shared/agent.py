...
class AgentExecutor(Chain):
    ...
    def _call(
        ...
        while self._should_continue(iterations, time_elapsed):
            ...
            context = next_step_output[0][-1]['answer'] if next_step_output[0][-1]['answer'] else next_step_output[0][-1] #ESTO NO SIRVE CON STRINGS
            question = inputs['input']
            prompt = 'Given the following context, is the question below accurate and valid? Only Answer with "yes" or "no".' + '\nContext: ' + context + '\nQuestion: ' + question
            validation = model.invoke(prompt)
            
            if(validation.content == 'Yes.'):
                next_step_output = AgentFinish({"output": context}, context)
